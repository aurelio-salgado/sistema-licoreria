const PUBLIC_COLUMNS=`r.id_respaldo,r.nombre_archivo,r.tamano_bytes,r.tipo,r.operacion,r.estado,r.archivo_disponible,r.mensaje_resultado,r.fecha_operacion,r.fecha_finalizacion,r.id_respaldo_origen,r.id_respaldo_preventivo,u.id_usuario,CONCAT(u.nombre,' ',u.apellido) usuario_nombre,u.nombre_usuario`;
function where(f){const c=[],v=[];for(const [value,sql] of [[f.type,'r.tipo=?'],[f.operation,'r.operacion=?'],[f.status,'r.estado=?']])if(value){c.push(sql);v.push(value);}if(f.dateFrom){c.push('r.fecha_operacion>=?');v.push(`${f.dateFrom} 00:00:00`);}if(f.dateTo){c.push('r.fecha_operacion<DATE_ADD(?,INTERVAL 1 DAY)');v.push(`${f.dateTo} 00:00:00`);}return{sql:c.length?`WHERE ${c.join(' AND ')}`:'',values:v};}
