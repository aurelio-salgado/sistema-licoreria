const CLIENT_STATES = new Set(['activo', 'inactivo']);
...
function rejectFinalConsumerField(body) {
  if (Object.prototype.hasOwnProperty.call(body || {}, 'es_consumidor_final')) {
    throw validationError(
      'es_consumidor_final no puede modificarse desde este CRUD',
    );
  }
}

function validateClientInput(body) {
  rejectFinalConsumerField(body);
  return {
    name: validateRequiredText(body?.nombre, 'El nombre', 150),
    identification: validateOptionalText(
      body?.identificacion,
      'La identificación',
      50,
    ),
...
function validateState(value) {
  if (typeof value !== 'string' || !CLIENT_STATES.has(value.trim())) {
