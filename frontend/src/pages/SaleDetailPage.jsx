...
export function SaleDetailPage() {
  ...
  const loadCatalogs = useCallback(async () => {
    if (!canEdit) return
    setCatalogError('')
    try { const [clientRows, productRows] = await Promise.all([loadAllActive(clientsApi.list, 'clients'), loadAllActive(productsApi.list, 'products')]); setClients(clientRows); setProducts(productRows) }
    catch (requestError) { setCatalogError(requestError.message || 'No fue posible cargar clientes o productos.') }
  }, [canEdit])
  ...
  return <div className="page-stack sale-detail-page">
    ...
    {editor?.type === 'item' && <Modal title={editor.item ? 'Editar línea' : 'Agregar producto'} onClose={closeEditor} busy={saving} wide><SaleItemForm key={editor.item?.id_detalle_venta ?? 'new'} item={editor.item} products={products} busy={saving} onCancel={closeEditor} onSubmit={saveItem} /></Modal>}
