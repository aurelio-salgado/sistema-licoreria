...
export function BackupsPage() {
  ...
  const restore = async (event) => { event.preventDefault(); setBusy('restore'); try { await backupsApi.restore(restoreItem.id_respaldo, confirmation); setRestoreItem(null); setConfirmation(''); setDialog({ title: 'Restauración completada', message: 'La restauración finalizó. Inicia sesión nuevamente.' }); await load() } catch (requestError) { setDialog({ title: 'Restauración no disponible', message: requestError.message }) } finally { setBusy('') } }
  ...
  return <div className="page-stack backups-page">
    ...
    {restoreItem && <Modal title="Restauración crítica" busy={busy === 'restore'} onClose={closeRestore} descriptionId="restore-warning" initialFocusRef={restoreConfirmationRef}><form className="restore-form" onSubmit={restore}><p id="restore-warning" className="inline-alert inline-alert--warning">Se reemplazarán los datos actuales y se perderán los cambios posteriores al respaldo. Primero se creará un respaldo preventivo. Todos los usuarios deberán iniciar sesión nuevamente.</p><label className="filter-field" htmlFor="restore-confirmation"><span>Escribe RESTAURAR para continuar</span><input ref={restoreConfirmationRef} id="restore-confirmation" className="form-control" autoComplete="off" value={confirmation} onChange={(e) => setConfirmation(e.target.value)} /></label><div className="modal-footer"><button className="button button--secondary" type="button" onClick={closeRestore}>Cancelar</button><button className="button button--danger" type="submit" disabled={confirmation !== 'RESTAURAR' || busy === 'restore'}>{busy === 'restore' ? 'Restaurando…' : 'Restaurar'}</button></div></form></Modal>}
