...
function buildSupervisionQuery(filters) {
  const query = new URLSearchParams({ page: String(filters.page), limit: String(filters.limit) })
...
export const cashApi = {
  ...
  listSupervision: (filters) => api.get(`/cash/supervision?${buildSupervisionQuery(filters)}`),
  getSupervisionById: (id) => api.get(`/cash/supervision/${id}`),
